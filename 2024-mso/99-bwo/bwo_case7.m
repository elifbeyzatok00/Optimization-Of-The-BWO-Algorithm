function [bestSolution, bestFitness, iteration]=bwo_case6(fhd, dimension, maxIteration, fNumber)
...
while T <= Max_it
    ...
    for i = 1:Npop
        if kk(i) > 0.5 % exploration phase
            tournamentSize = 3;
            candidates = randi(Npop , [1 , tournamentSize]);
            [~, bestCandidateIdx] = min(fit(candidates));
            r1 = candidates(bestCandidateIdx);
            r2 = rand();
        ...
        else  % exploitation phasehttps://g.co/gemini/share/6a8ab359641bhttps://g.co/gemini/share/6a8ab359641b
            r3 = rand(); 
            r4 = rand(); 
            C1 = 2*r4*(1-T/Max_it);
            RJ = ceil(Npop*rand);   % Roulette Wheel Selection
